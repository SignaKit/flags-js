 ...
 * All platform SDKs import this implementation to guarantee identical
 * bucketing results across server, browser, and mobile runtimes.
 */
...
export function hashToBucket(salt: string, userId: string): number {
  const hash = murmur3_32(`${salt}:${userId}`)
