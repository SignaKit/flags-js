...
describe('evaluateFlag — archived and disabled states', () => {
  it('returns null for archived flags (excluded from decisions map)', () => {
...
describe('evaluateFlag — allowlist', () => {
  ...
  it("returns allowlisted 'off' when explicitly set for a user", () => {
...
describe('evaluateFlag — traffic allocation', () => {
  it('places all users in traffic when trafficPercentage=100', () => {
    ...
    expect(evaluateFlag(flag, 'any-user')!.variationKey).toBe('on')
    expect(evaluateFlag(flag, 'any-user')!.ruleKey).toBe('rule-all')
  })

  it('excludes all users from traffic when trafficPercentage=0', () => {
    const flag = makeFlag({
      key: 'zero-traffic',
      allocation: { ranges: [{ variation: 'off', start: 0, end: 9999 }] },
      rules: [
...
describe('evaluateFlag — audience targeting', () => {
  ...
  it('falls through to default when user does not match the audience', () => {
  ...
  it('falls through to default when user has no attributes', () => {
    const result = evaluateFlag(flag, 'no-attrs-user')
    expect(result!.variationKey).toBe('off')
    expect(result!.ruleKey).toBeNull()
  })
})
...
describe('evaluateFlag — default allocation', () => {
  it('uses default allocation when no rules are defined', () => {
    const flag = makeFlag({
      key: 'no-rules',
      allocation: { ranges: [{ variation: 'on', start: 0, end: 9999 }] },
    })
  ...
  it('returns off fallback when default allocation has no matching range', () => {
    const flag = makeFlag({ key: 'empty-alloc', allocation: { ranges: [] } })
    const result = evaluateFlag(flag, 'user-1')
    expect(result!.variationKey).toBe('off')
    expect(result!.enabled).toBe(false)
  })
...
describe('evaluateFlag — variable resolution', () => {
  ...
  it('returns default variable values for a variation with no overrides', () => {
...
describe('evaluateFlag — deterministic bucketing', () => {
  it('always assigns the same variation to the same user across multiple calls', () => {
    const flag = makeFlag({
      key: 'determinism',
      allocation: {
        ranges: [
  ...
  it('splits users across variations roughly evenly with 50/50 allocation', () => {
    const flag = makeFlag({
      key: 'fifty-fifty',
      rules: [
        {
    ...
    const variations = Array.from({ length: 1000 }, (_, i) =>
      evaluateFlag(flag, `user-${i}`)!.variationKey
    )
    const control = variations.filter((v) => v === 'control').length
    const treatment = variations.filter((v) => v === 'treatment').length
...
describe('evaluateAllFlags', () => {
  ...
  it('returns an empty object when all flags are archived', () => {
    const allArchived: ProjectConfig = {
      ...config,
      flags: [makeFlag({ key: 'x', status: 'archived' })],
    }
    expect(evaluateAllFlags(allArchived, 'user-1')).toEqual({})
  })

  it('returns an empty object when there are no flags', () => {
    const empty: ProjectConfig = { ...config, flags: [] }
    expect(evaluateAllFlags(empty, 'user-1')).toEqual({})
  })
})
