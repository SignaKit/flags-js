...
describe('matchesCondition — equals / not_equals', () => {
  ...
  it('equals: matches numeric values', () => {
    const cond: AudienceCondition = { attribute: 'score', operator: 'equals', value: 42 }
    expect(matchesCondition(cond, { score: 42 })).toBe(true)
    expect(matchesCondition(cond, { score: 43 })).toBe(false)
  })
  ...
  it('returns false when attribute is missing from user attributes', () => {
    const cond: AudienceCondition = { attribute: 'plan', operator: 'equals', value: 'premium' }
    expect(matchesCondition(cond, {})).toBe(false)
  })

  it('returns false when attributes are undefined', () => {
    const cond: AudienceCondition = { attribute: 'plan', operator: 'equals', value: 'premium' }
...
describe('matchesCondition — numeric comparisons', () => {
  ...
  it('greater_than_or_equals: inclusive lower bound', () => {
    expect(matchesCondition(age('greater_than_or_equals', 18), { age: 18 })).toBe(true)
    expect(matchesCondition(age('greater_than_or_equals', 18), { age: 19 })).toBe(true)
    expect(matchesCondition(age('greater_than_or_equals', 18), { age: 17 })).toBe(false)
  })

  it('less_than_or_equals: inclusive upper bound', () => {
    expect(matchesCondition(age('less_than_or_equals', 18), { age: 18 })).toBe(true)
    expect(matchesCondition(age('less_than_or_equals', 18), { age: 17 })).toBe(true)
    expect(matchesCondition(age('less_than_or_equals', 18), { age: 19 })).toBe(false)
  })

  it('numeric operators return false when attribute is a string', () => {
    expect(
      matchesCondition(age('greater_than', 18), { age: '25' as unknown as number })
    ).toBe(false)
  })
})
...
describe('matchesCondition — contains / not_contains', () => {
  ...
  it('contains: false when types do not match', () => {
    const cond: AudienceCondition = { attribute: 'score', operator: 'contains', value: 'high' }
    expect(matchesCondition(cond, { score: 99 })).toBe(false)
  })
...
describe('matchesAudience', () => {
  it('returns true when all conditions match (AND logic within an audience)', () => {
  ...
  it('returns false when any single condition fails', () => {
    const audience: ConfigRuleAudience = {
      conditions: [
        { attribute: 'plan', operator: 'equals', value: 'premium' },
        { attribute: 'age', operator: 'greater_than_or_equals', value: 18 },
  ...
  it('returns true for an audience with no conditions (matches all users)', () => {
...
describe('matchesAudiences', () => {
  it('returns true when audiences are undefined (matches all users)', () => {
    expect(matchesAudiences(undefined, 'any', {})).toBe(true)
  })

  it('returns true when audiences array is empty (matches all users)', () => {
    expect(matchesAudiences([], 'all', {})).toBe(true)
  })

  it('any: true when at least one audience matches (OR logic)', () => {
    ...
    expect(matchesAudiences(audiences, 'any', { plan: 'enterprise' })).toBe(true)
    expect(matchesAudiences(audiences, 'any', { plan: 'free' })).toBe(false)
  })

  it('all: true only when every audience matches (AND logic)', () => {
    const audiences: ConfigRuleAudience[] = [
      { conditions: [{ attribute: 'plan', operator: 'equals', value: 'premium' }] },
    ...
    expect(matchesAudiences(audiences, 'all', { plan: 'free', verified: true })).toBe(false)
  })
