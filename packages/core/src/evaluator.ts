 ...
 *
 * This is the canonical implementation shared across all platform SDKs.
 * All SDKs must produce identical decisions for the same userId / attributes / config.
 */
...
export function evaluateFlag(
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (matchesAudiences(rule.audiences, rule.audienceMatchType, attributes)) {
      ...
      // trafficPercentage is 0-100; multiply by 100 to get bucket threshold (0-10000)
